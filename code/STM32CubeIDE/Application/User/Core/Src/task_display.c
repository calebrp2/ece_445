...
static const char *s_wave_labels[RF_N_CLASSES] = {
    "SINE", "SQUARE", "SAWT", "TRI", "P25"
};
...
static void render_status(void)
{
    ...
    const char *wave_str = (rf.confidence_pct > 0U)
                           ? s_wave_labels[rf.waveform]
                           : "--";

    char buf[80];
    snprintf(buf, sizeof(buf), "%s-%s %s F:%luHZ A:%ldMV %s %s:%u%%",
             bot_str, top_str,
             mode_str, freq_int, (long)amp_mv,
             fit_ok ? "OK" : "--",
             wave_str, (unsigned)rf.confidence_pct);
